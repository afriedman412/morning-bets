# RESUME ARCHIVE — days six to sixteen (2026-08-21 to 2026-08-29)

Split out of `RESUME.md` on 2026-08-30, when that file passed 3,700 lines and
could no longer be read in one pass.

**NOTHING HERE IS DELETED AND SOME OF IT IS STILL LOAD-BEARING.** What it is
NOT is current: figures predate several engine changes, and two — the run
deficit — were retracted on 2026-08-30. Re-derive any number here before
reasoning from it.

**THE THINGS IN THIS FILE STILL WORTH KNOWING**, so you can decide whether
to open it:

  * THE BABIP DENOMINATOR FIX. `bip = outs + hits - K - HR` counts OUTS, and
    a double play is one ball in play and two outs. One error visible in
    exactly one channel, because BABIP's level reaches the sim as an
    absolute rate while the others cancel in log5.
  * THE FIVE-WAY TRAFFIC DECOMPOSITION. There are only five ways to reach
    base and nobody had split them for six days. "Too few baserunners" was
    the wrong diagnosis for a week.
  * FOUR CANDIDATES SCREENED ON RESIDUALS, all dead, minutes each —
    handedness, catcher identity, arsenal, lineup slot due up. The pattern
    to copy: a residual correlation over all starts, no simulation.
  * ARSENAL IS HARMFUL, NOT NULL, with a control firing at +113 sigma.
  * THE HIT MIX IS CORRECT and does not need re-measuring.
  * "HOW MUCH OF THIS IS ALREADY INSIDE THE PITCHER'S OWN RATES?" — the
    question that killed team defence and park. An observed spread across
    clubs is not an opportunity.
  * THE INVESTIGATION PROTOCOL, whose canonical copy is at the END of
...
# DAYS THIRTEEN TO SIXTEEN — moved out of RESUME 2026-08-30
