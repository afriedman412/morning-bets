...
STATS = ("outs", "k", "er", "h", "bb", "hr")
_COL = {"outs": "o", "k": "k", "er": "er", "h": "h", "bb": "bb", "hr": "hr"}
...
LINES = {"outs": (14.5, 15.5, 17.5, 18.5, 20.5),
         ...
         "h": (4.5, 5.5, 6.5),
         "bb": (1.5, 2.5),
         # home runs are where temperature and wind should land hardest —
         # they are a batted-ball distance question and nothing else here is
         "hr": (0.5, 1.5)}
...
def _one(args):
    ...
    draws = [cal.replay(pair, lg, _PENS, rng, track=(3, 5, 7))
             for _ in range(n_sims)]
    # TEAM TOTALS, off the same games. `prefix_side` is {inning: (away
    # score, home score)} and a Side's runs are runs ALLOWED, so the crossing
    # is already undone there. This is the STATED PRODUCT and it is the
    # quantity the crossed-lineup bug hit hardest: a team's simulated runs
    # were being generated by its OWN hitters.
    team = {"game_id": pair[0][0]["game_id"], "date": pair[0][0]["date"],
            "away_team": pair[0][0]["team"], "home_team": pair[1][0]["team"],
            "_team_row": True}
    for n in (3, 5, 7):
        av = [d.prefix_side[n][0] for d in draws if n in d.prefix_side]
        hv = [d.prefix_side[n][1] for d in draws if n in d.prefix_side]
        if not av:
            continue
        for tag, v in (("away", av), ("home", hv)):
            team[f"m_{tag}_f{n}"] = st.mean(v)
            team[f"w_{tag}_f{n}"] = st.pvariance(v)
    team["m_away_full"] = st.mean(d.away for d in draws)
    team["w_away_full"] = st.pvariance([d.away for d in draws])
    team["m_home_full"] = st.mean(d.home for d in draws)
    team["w_home_full"] = st.pvariance([d.home for d in draws])
    out = [team]
...
def team_report(rows) -> None:
    """Per-side TEAM TOTALS — the stated product in AF_PLAN.

    Scored the same way as the starter lines: how much between-GAME
    variation really exists, and how much of it we produce. A team total is
    the quantity the crossed-lineup bug hit hardest, because a club's runs
    are generated by the lineup facing the opposing pitcher.
    """
    from src.context.sources import innings as inn_src
    trows = [r for r in rows if r.get("_team_row")]
    if not trows:
        return
    print(f"\n  TEAM TOTALS — {len(trows)} games, both sides\n")
    print(f"  {'':<8}{'actual sd':>11}{'our within':>12}{'implied':>9}"
          f"{'our spread':>12}{'share':>8}{'corr':>8}{'level':>9}")
    for n in (3, 5, 7, None):
        act = inn_src.prefix_totals(n) if n else None
        pred, w, a = [], [], []
        for r in trows:
            gid = r["game_id"]
            for tag in ("away", "home"):
                key = f"m_{tag}_f{n}" if n else f"m_{tag}_full"
                wk = f"w_{tag}_f{n}" if n else f"w_{tag}_full"
                if key not in r:
                    continue
                if n:
                    if gid not in act:
        ...
        if len(pred) < 50:
            continue
        sa, sm = st.pstdev(a), st.pstdev(pred)
        between = max(st.pvariance(a) - st.mean(w), 0) ** 0.5
        ma, mm = st.mean(a), st.mean(pred)
        r_ = (sum((x - mm) * (y - ma) for x, y in zip(pred, a))
              / (len(a) * sm * sa)) if sm and sa else 0.0
        lbl = f"F{n}" if n else "full"
        print(f"  {lbl:<8}{sa:>11.2f}{st.mean(w) ** 0.5:>12.2f}"
              f"{between:>9.2f}{sm:>12.2f}"
              f"{(sm / between if between else 0):>8.0%}{r_:>8.3f}"
              f"{mm - ma:>+9.2f}")
    print("\n  'level' is our mean minus the real mean, in runs per side.")
    ...
    if not _FULL:
        ...
        with db.connect() as c:
            for r in c.execute(
                ...
                _FULL[(r["game_id"], "away")] = r["away_score"]
                _FULL[(r["game_id"], "home")] = r["home_score"]
    return _FULL


def report(rows) -> None:
    rows = [r for r in rows if not r.get("_team_row")]
    print(f"\n  {len(rows)} starts")
...
def main():
    ...
    # after the cutoff.
    # Turning the home/road adjustment OFF is what separates "already
    # correctly sized" from "irrelevant". With it on, a null on `is_home`
    # in the residual only says the applied correction is right.
    if "--nohomeroad" in sys.argv:
        cal.USE_HOME_ROAD = False
        sys.argv = [a for a in sys.argv if a != "--nohomeroad"]
    # PARK, done correctly for the first time. `--park` turns on BOTH
    # neutralisation and application: a pitcher's season rates already
    # contain his own park for roughly half his starts, so applying a park
    # factor on top of raw rates counts the home park about one and a half
    # times and mis-bases the road side. The pair is the only honest test.
    if "--park" in sys.argv:
        cal.NEUTRALISE_PARK = True
        cal.USE_PARK = True
        sys.argv = [a for a in sys.argv if a != "--park"]
    ...
    if not cal.USE_HOME_ROAD:
        path = path.replace(".json", "_nohr.json")
    if cal.USE_PARK:
        path = path.replace(".json", "_park.json")
