"""CROSS-VALIDATE THE ITERATED BOUNDARY TABLE ON THE OUTS LADDER. TODO 7a.

    venv/bin/python -m scratchpad.hz_cv_bnd [n_sims]

QUESTION    `hz_iter.py` re-solved `PITCH_HAZARD_BND` against the model's
            own states. The STATIC counted table lost this exact test:
            dead heat on all-line error and +0.18 mean-outs overshoot in
            every season. Does the iterated one hold the middle band,
            leave the long lines alone, and keep the mean?

Same harness as `hz_cv.py` except the toggle is `USE_PITCH_HAZARD_BND`
(the counted MID backbone stays ON in both arms — it shipped). "off" is
therefore the SHIPPED state and "ON" is the candidate. The table under
test is whatever `sim.PITCH_HAZARD_BND` holds — paste the iterated values
into sim.py before running.

PRE-REGISTERED BAR, written before the run: the middle band must not get
worse in more than one fold, mean outs must move TOWARD real (the shipped
state runs short) without flipping past it by more than the shortfall it
closes, and the long lines must not double. The 2026 fold is the clean
one; 2023-2025 replicate (the targets pooled their May-June rows).
"""
...
def main(argv):
    ...
    for yr, cut in FOLDS:
        ...
        b0 = sum(abs(d0[ln]) for ln in BAND) / len(BAND)
        b1 = sum(abs(d1[ln]) for ln in BAND) / len(BAND)
        print(f"  {yr}  {len(gids)} games, {n:,} real starts   band "
              f"{b0:.4f} -> {b1:.4f}   "
              f"{'IMPROVES' if b1 < b0 else 'WORSE'}")

    print("\n  MIDDLE BAND o12.5-o17.5, mean |gap|, shipped -> BND\n")
    print(f"  {'fold':<8}{'shipped':>10}{'BND':>10}{'change':>12}")
    for yr, _ in FOLDS:
        d0, *_ = res[(yr, False)]
        d1, *_ = res[(yr, True)]
        b0 = sum(abs(d0[ln]) for ln in BAND) / len(BAND)
    ...
    print("\n  LONG LINES o18.5/o20.5\n")
    print(f"  {'fold':<8}{'shipped':>10}{'BND':>10}{'change':>12}")
    for yr, _ in FOLDS:
        d0, *_ = res[(yr, False)]
        d1, *_ = res[(yr, True)]
        l0 = sum(abs(d0[ln]) for ln in (18.5, 20.5)) / 2
        l1 = sum(abs(d1[ln]) for ln in (18.5, 20.5)) / 2
        print(f"  {yr:<8}{l0:>10.4f}{l1:>10.4f}{l1 - l0:>+12.4f}")
    print("\n  MEAN OUTS, model vs real\n")
    print(f"  {'fold':<8}{'shipped':>10}{'BND':>10}{'real':>10}")
    for yr, _ in FOLDS:
